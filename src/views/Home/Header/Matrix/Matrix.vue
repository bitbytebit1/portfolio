<template>
  <div id="container">
    ...
    <div
      id="overlay"
      class="white--text display-4 font-weight-light"
    >
      BitByteBit
    </div>
  </div>
...
export default {
  mounted () {
    ...
    var c = document.getElementById('wakeup')
    ...
    // var matrix = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890!@#$%^&*()_+ÖÄÅÉé来到制造想像维护制作看法牠们基本态度问有点表演办理由了消失照顾关怀模式月行动要求最佳男子严重女子호주테이프주전자울음보살피다학술사회자부동산공연하다사생활유리어쩜현재말티셔츠마음노랫소리너머다하다젓가락참외아가씨육체가져가다배꼽유학생'
    let matrix = []
    for (let i = 1000; i < 100000; i++) {
      matrix.push(String.fromCharCode('0x' + i))
    }
